...
# FIX #6: Убрал 2>/dev/null для stderr, чтобы не скрывать ошибки установки пакетов
safe_cmd() {
    ...
    if [[ "$1" == "sudo" ]]; then
        ...
        timeout "$timeout_sec" sudo "$@"
    fi
    return $?
}

# FIX #10: Удалена функция safe_sudo_cmd как дублирующая safe_cmd
        ...
        if [[ -n "$nvme_list" ]]; then
            ...
            echo "$nvme_list" | tail -n +2 | while read -r line; do
                ...
                if [[ -e "/dev/${nvme_dev}" ]]; then
                    local health=$(safe_cmd 10 nvme smart-log "/dev/${nvme_dev}" | grep -E "critical_warning|temperature|available_spare" || echo "")
                    if [[ -n "$health" ]]; then
...
main() {
    ...
    # FIX #1, #11: Запуск run_scan без subshell для сохранения глобальных массивов + line-buffered tee
    shopt -s lastpipe 2>/dev/null || true
    run_scan > >(tee --line-buffered "$OUTPUT_FILE" >/dev/null) 2>&1
